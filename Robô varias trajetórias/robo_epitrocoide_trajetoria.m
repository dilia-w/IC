...
% Parâmetros da epitrocoide
R = 1.5; % Raio do círculo maior
r = 0.5; % Raio do círculo menor
d = 0.5; % Distância do ponto ao centro do círculo menor

% Calcular coordenadas da epitrocoide
x_epitrochoid = (R + r) * cos(theta_t) - d * cos(((R + r) / r) * theta_t) + circle_center_x;
y_epitrochoid = (R + r) * sin(theta_t) - d * sin(((R + r) / r) * theta_t) + circle_center_y;
...
for i = 1:num_points
    ...
    x_end_effector(i) = x_epitrochoid(i);
    y_end_effector(i) = y_epitrochoid(i);
    ...
    if inside_workspace
        ...
        subplot(2,2,1);
        plot(x_epitrochoid, y_epitrochoid, 'r--', 'LineWidth', 1.5);
        hold on;
