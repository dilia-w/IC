...
% Parâmetros da hipocicloide
R = 3; % Raio do círculo maior
r = 1; % Raio do círculo menor

% Calcular coordenadas da hipocicloide
x_hypocycloid = (R - r) * cos(theta_t) + r * cos(((R - r) / r) * theta_t) + circle_center_x;
y_hypocycloid = (R - r) * sin(theta_t) - r * sin(((R - r) / r) * theta_t) + circle_center_x;
...
for i = 1:num_points
    ...
    x_end_effector(i) = x_hypocycloid(i);
    y_end_effector(i) = y_hypocycloid(i);
    ...
    if inside_workspace
        ...
        subplot(2,2,1);
        plot(x_hypocycloid, y_hypocycloid, 'r--', 'LineWidth', 1.5);
        hold on;
