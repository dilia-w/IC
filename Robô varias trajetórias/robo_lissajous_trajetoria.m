...
% Parâmetros da curva de Lissajous
A = 2; % Amplitude no eixo X
B = 1.5; % Amplitude no eixo Y
a = 3; % Frequência no eixo X
b = 2; % Frequência no eixo Y
delta = pi/2; % Deslocamento de fase

% Calcular coordenadas da curva de Lissajous
x_lissajous = A * sin(a * theta_t + delta) + circle_center_x;
y_lissajous = B * sin(b * theta_t) + circle_center_y;
...
for i = 1:num_points
    ...
    x_end_effector(i) = x_lissajous(i);
    y_end_effector(i) = y_lissajous(i);
    ...
    if inside_workspace
        ...
        subplot(2,2,1);
        plot(x_lissajous, y_lissajous, 'r--', 'LineWidth', 1.5);
        hold on;
