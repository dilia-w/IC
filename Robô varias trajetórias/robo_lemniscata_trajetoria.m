...
a = 2; % Parâmetro que define o tamanho da curva

% Calcular coordenadas da lemniscata
x_lemniscate = a * cos(theta_t) ./ (1 + sin(theta_t).^2) + circle_center_x;
y_lemniscate = a * cos(theta_t) .* sin(theta_t) ./ (1 + sin(theta_t).^2) + circle_center_y;
...
for i = 1:num_points
    ...
    x_end_effector(i) = x_lemniscate(i);
    y_end_effector(i) = y_lemniscate(i);
    ...
    if inside_workspace
        ...
        subplot(2,2,1);
        plot(x_lemniscate, y_lemniscate, 'r--', 'LineWidth', 1.5);
        hold on;
