...
% Parâmetros da roseta
num_petals = 5; % Número de pétalas
rosette_radius = 1.5;

% Calcular coordenadas da roseta
x_rosette = rosette_radius * cos(num_petals * theta_t) .* cos(theta_t) + circle_center_x;
y_rosette = rosette_radius * cos(num_petals * theta_t) .* sin(theta_t) + circle_center_y;
...
for i = 1:num_points
    ...
    x_end_effector(i) = x_rosette(i);
    y_end_effector(i) = y_rosette(i);
    ...
    if inside_workspace
        ...
        subplot(2,2,1);
        plot(x_rosette, y_rosette, 'r--', 'LineWidth', 1.5);
        hold on;
