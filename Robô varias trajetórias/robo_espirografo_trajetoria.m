...
% Parâmetros do espirógrafo
R = 1.5; % Raio do círculo maior
r = 0.5; % Raio do círculo menor
d = 1.5; % Distância do ponto ao centro do círculo menor

% Calcular coordenadas do espirógrafo
x_spirograph = (R - r) * cos(theta_t) + d * cos(((R - r) / r) * theta_t) + circle_center_x;
y_spirograph = (R - r) * sin(theta_t) - d * sin(((R - r) / r) * theta_t) + circle_center_y;
...
for i = 1:num_points
    ...
    x_end_effector(i) = x_spirograph(i);
    y_end_effector(i) = y_spirograph(i);
    ...
    if inside_workspace
        ...
        subplot(2,2,1);
        plot(x_spirograph, y_spirograph, 'r--', 'LineWidth', 1.5);
        hold on;
